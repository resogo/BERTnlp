...
class NerProcessor(DataProcessor):
    ...
    def get_labels(self):
        ...
        :return:
        """
        return ["[PAD]","PATIENT", "DATE", "HOSPITAL", "PHONE", "DOCTOR", "LOCATION", "AGE", "ID", "NONE", "X","[CLS]","[SEP]"]
...
def create_model(bert_config, is_training, input_ids, mask,
    ...
    output_weights = tf.get_variable("output_weights", [batch_size,num_labels, hidden_size],initializer=tf.contrib.layers.xavier_initializer(uniform=True,seed=None,dtype=tf.dtypes.float32))
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
    ...
    def model_fn(features, labels, mode, params):
        ...
        bert_config, is_training, input_ids, mask, segment_ids, label_ids,
        ...
        #train_writer = tf.summary.FileWriter(FLAGS.output_dir + '/summary/train',graph)
        #test_writer = tf.train.SummaryWriter(FLAGS.summaries_dir + '/summary/test')
...
def main(_):
    ...
    if FLAGS.do_eval:
        ...
        with open(output_eval_file,"w") as wf:
            ...
            p,r,f,a = metrics.calculate(confusion_matrix,len(label_list)-1)
            logging.info("***********************************************")
            ...
            wf.write("F = "+str(f)+"\n")
            wf.write("A = "+str(a)+"\n")
            wf.write("  Num examples = "+ str(len(eval_examples))+"\n")
            wf.write("  Batch size = "+str(FLAGS.eval_batch_size)+"\n")
            wf.write("  Iterations Per Loop = "+str(FLAGS.iterations_per_loop))
            
            wf.write("***********************************************\n")
            for i in range(len(label_list)-1):
                for r in range(len(label_list)-1):
                    wf.write(str(confusion_matrix[i][r])+"  " )
                wf.write("\n")
